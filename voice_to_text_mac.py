...
DEFAULT_SETTINGS = {
    "api_key": "",
    "hotkey": "cmd+space",  # macOS friendly hotkey
    "microphone": "",
    "language": "ru",
    "ui_lang": "ru",
    "auto_paste": True,
...
class VoiceToTextApp(ctk.CTk):
    ...
    def set_autostart(self, enable):
        """Enable or disable autostart with macOS LaunchAgents."""
        try:
...
💡 Ключ бесплатный, лимит ~14,400 запросов/день""",
    ...
    def open_terms_file(self):
        ...
        if os.path.exists(terms_path):
            subprocess.Popen(['open', '-t', terms_path])
        else:
    ...
    def _format_hotkey_display(self, hotkey):
        """Format hotkey for display with macOS symbols."""
        display = hotkey.upper()
        display = display.replace("CMD", "\u2318")
    ...
    def change_hotkey(self):
        self.hotkey_label.configure(text="Press keys...", text_color=COLORS["warning"])
        ...
        def on_press(key):
            ...
            self.captured_hotkey = list(self.current_keys)

        def on_release(key):
            if len(self.captured_hotkey) >= 2:
                parts = []
                for k in self.captured_hotkey:
                    if k == Key.cmd:
                        parts.append('cmd')
                    elif k == Key.ctrl:
                        parts.append('ctrl')
                        ...
                        parts.append(k.name)
                hotkey = '+'.join(parts)
                self.after(0, lambda: self._set_hotkey(hotkey))
                return False
            self.current_keys.discard(key)
            return True

        capture_listener = pynput_keyboard.Listener(on_press=on_press, on_release=on_release)
        capture_listener.start()

    def _set_hotkey(self, hk):
        self.settings["hotkey"] = hk
        self.save_settings()
        self.hotkey_label.configure(text=self._format_hotkey_display(hk), text_color=COLORS["accent"])
        self.setup_hotkey()
    ...
    def start_recording(self):
        ...
        # macOS: no need to save focused window, pyautogui handles it
    ...
    def handle_result(self, text):
        ...
        # Auto-paste using pyautogui (Cmd+V on macOS)
...
if __name__ == "__main__":
    print("=" * 50)
    print(f"VTT @SAINT4AI - Voice to Text v{APP_VERSION} (macOS)")
    print("=" * 50)
    app = VoiceToTextApp()
    app.mainloop()
